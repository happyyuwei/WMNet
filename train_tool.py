...
def save_images(image_list, title_list, image_dir, seq=""):
    ...
    for i in range(len(title_list)):
        dir = os.path.join(image_dir, str(seq) +
                           "_" + title_list[i] + ".png")
...
def read_image(path, width, height, change_scale=False, binary=False):
    ...
    image = image[:, :, 0:3]
    image = np.resize(image, [1, width, height, 3])
...
class LogTool:
    ...
    def save_image_plt(self, model, test_input, tar):
        ...
        output_image_file = os.path.join(
            self.image_dir, str(self.current_epoch) + ".png")
    ...
    def save_loss(self, **kwargs):
        """
        update the save keys t all kinds.
        @author
        @update 2019.9.13
        save loss
        :param gen_loss:
        ...
        with open(self.train_log_dir, "a") as f:
            ...
            for key in kwargs:
                if kwargs[key] != None:
                    content = content + ",{}={}".format(key, kwargs[key])
