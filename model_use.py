    ...
    def __init__(self, model_path, key_enable=False):
        ...
        if key_enable == False:
            checkpoint = tf.train.Checkpoint(
                train_optimizer=train_optimizer, encoder=self.encoder, decoder=self.decoder)
        else:
            checkpoint = tf.train.Checkpoint(
                train_optimizer=train_optimizer, encoder=self.encoder, decoder=self.decoder, key_encoder=self.key_encoder)
        print("load model....")
...
if __name__ == "__main__":
    ...
    model = EncoderDecoder("./experiments/key_sensitive/test1/train_model1", key_enable=True)
    ...
    # decode feature
    wm = model.decode_from_image("./experiments/key_sensitive/test1/data/142_EN.png",secret_key_path="./watermark/key.npy")
